...
const Stats = (props) => {
    ...
    useEffect(() => {
      ...
      axios.get(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/Stats/username=${Username}&page=${pageNumber}&limit=${10}`)
        .then(res => {
          let data = res.data
          let temp_Games = [];
          if (data) {
    ...
    useEffect(() => {
      ...
      axios.get(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/Stats/username=${Username}/count`)
      .then(res => {
        let data = res.data[0]
        if (data.error) {
          notification.error({
