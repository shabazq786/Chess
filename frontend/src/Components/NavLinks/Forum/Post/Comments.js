...
const Comments = (props) => {
  ...
  function onEditComment(comment_id) {
    ...
    setEdit(true) 
      ...
      axios.put(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/commentid=${comment_id}/edit`, {text:commentid.body})
      .then(res => {
  ...
  function onDeleteComment(commentid) {
    axios.delete(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/commentid=${commentid}/delete`)
      .then(res => {
        const data = res.data[0]
        if (data.error) {
          notification.error({
  ...
  function onSubmitHandler(e) {
    ...
    axios.post(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/postid`, { post_id:props.postid_path, body:ReplyText, posted_by: props.Username, parent_id:props.commentid.pk})
    .then(res => {
      const data = res.data[0]
      if (data.error) {
        notification.error({
  ...
  useEffect(() => {
    axios.get(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/postid=${props.postid}&parent=${props.commentid.pk}/count`)
    .then(res => {
      const data = res.data[0]
      if (data.error){
        notification.error({
  ...
  useEffect(() => {
    ...
    if (showReplies) {
      axios.get(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/postid=${props.postid}&username=${User}&parent=${props.commentid.pk}&page=1&limit=10000`)
        .then(res => {
          const data = res.data
          if (data.error)  {
            notification.error({
  ...
  function toggleLikes() {
    ...
    axios.put(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/comments/ratingsedit`, {comment_id:props.commentid.pk, username:props.Username, rate:'likes'})
    .then(res => {
      const data = res.data
      if (data.error)  {
        notification.error({
  ...
  function toggleDisLikes() {
    ...
    axios.put(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/comments/ratingsedit`, {comment_id:props.commentid.pk, username:props.Username, rate:'dislikes'})
    .then(res => {
      const data = res.data
      if (data.error)  {
        notification.error({
  ...
  return ( 
    ...
    <Form onSubmit={onSubmitHandler} style={{display: showBox ? "": "none"}}>
      <div className="Form-child">
        <div style={{color: "red", display: props.isLoggedIn ? "none":""}}>Please Log In First!</div>
        <div>
