...
import {Pagination, Header, Icon, Segment} from 'semantic-ui-react'
...
const Post = (props) => {
  ...
  function onSubmitComment(e) {
    ...
    axios.post(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/postid`, { post_id:postid_path, body:text, posted_by: Username, parent_id:"None"})
      .then(res => {
  ...
  function onEditPost() {
    if (!Edit) {
      setEdit(true) 
        ...
        axios.put(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/postid=${postid}/edit`, {title:ref.current.title, body:ref.current.body})
        .then(res => {
          const data = res.data[0]
          if (data.error) {
            notification.error({
  ...
  function onDeletePost() {
    axios.delete(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/postid=${postid}/delete`)
    .then(res => {
      const data = res.data[0]
      if (data.error) {
        notification.error({
  ...
  useEffect(() => {
    if (!render) {
      ...
      axios.get(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/postid=${postid}`)
        .then(res => {
          const data = res.data[0]
  ...
  useEffect(() => {
    ...
    axios.get(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/postid=${postid}&username=${User}&parent=${parent_id}&page=${pageNumber}&limit=${pageSize}`)
      .then(res => {
        const data = res.data
        let temp_comments = []
  ...
  useEffect(() => {
    axios.get(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/postid=${postid}/count`)
      .then(res => {
        const data = res.data[0]
        if (data.error) {
          notification.error({
