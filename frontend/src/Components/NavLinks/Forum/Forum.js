...
const Forum = () => {
    ...
    useEffect(() => {
      axios.get(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/like=${matchedText}&page=${pageNumber}&limit=${10}`)
        .then(res => {
          const data = res.data
          let temp_posts = [];
          if (data) {
    ...
    useEffect(() => {
      axios.get(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/forum/posts/like=${matchedText}/count`)
      .then(res => {
    ...
    posts_table = 
      <Table striped fixed unstackable padded>
        <Table.Header>
          <Table.Row >
            <Table.HeaderCell style={{width:"70%"}}></Table.HeaderCell> 
            <Table.HeaderCell style={{width:"20%"}}> Last Modified By </Table.HeaderCell>
            <Table.HeaderCell style={{width:"10%"}}> Replies </Table.HeaderCell> 
          </Table.Row>
        </Table.Header>
        ...
        <Post show = { show } onHide = { onHidePost }/> 
          <Segment raised className="forum-container">
            ...
            <div className = "create-post" >
              ...
              <li className = "post-search">< Search onChange={onChangeSearch}placeholder = "Search For Posts.."/></li> 
            </div> 
            <div className = "posts-container" > { posts_table } </div> 
            <div className = "no-posts"
              style = {
              { display: posts.length ? 'none' : '' } } > No Posts Here! 
            </div>
