...
const Login = (props) => {
  ...
  function onSubmitHandler(e) {
    e.preventDefault();
    if (active) {
      axios.post(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/users/user=${Username}`, {password: Password })
        .then(res => {
      ...
      flag = true;
      axios.post(`http://chess-backend.eba-3fysfw6z.us-west-2.elasticbeanstalk.com/users/`, { username: Username.trim(), email: Email.trim(), password: Password, avatar: Image})
        .then(res => {
          const data = res.data[0]
